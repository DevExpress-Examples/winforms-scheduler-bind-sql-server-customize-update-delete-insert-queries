...
Namespace SchedulerBindDynamically
    ...
    Public Partial Class Form1
        ...
        Private Sub BindScheduler()
            ...
            Dim connection As SqlConnection = New SqlConnection("Data Source=.\SQLExpress;Initial Catalog=SchedulerBindDynamically;Integrated Security=SSPI")
            Dim selectCommand As SqlCommand = New SqlCommand("SELECT ID, StartTime, EndTime, Subject FROM CarScheduling", connection)
            dataSet = New DataSet()
        ...
        Private Sub Storage_AppointmentsModified(ByVal sender As Object, ByVal e As PersistentObjectsEventArgs)
            ...
            Catch ex As DBConcurrencyException
                Call MessageBox.Show("Concurrency violation:" & Microsoft.VisualBasic.Constants.vbCrLf & ex.Row(CStr("Subject")).ToString())
            End Try
        'dataSet.AcceptChanges();
        End Sub

        Private Sub Adapter_RowUpdated(ByVal sender As Object, ByVal e As SqlRowUpdatedEventArgs)
            If e.Status = UpdateStatus.Continue AndAlso e.StatementType = StatementType.Insert Then
                Dim id As Integer = 0
                Using cmd As SqlCommand = New SqlCommand("SELECT IDENT_CURRENT('CarScheduling')", dataAdapter.SelectCommand.Connection)
